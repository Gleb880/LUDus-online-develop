...
class Gallery {
    ...
    manageHTML() {
        ...
        if (this.settings.dots) {
            ...
            this.dotsNodes = this.dotsNode.querySelectorAll(`.${GalleryDotClassName}`);
        }
    ...
    dragging(evt) {
        this.dropX = evt.pageX;
        const dragShift = this.dragX -this.clickX;
        const easing = dragShift / 5;
        this.x = Math.max(Math.min(this.startX + dragShift, easing), this.maximusmX + easing);
    ...
    changeActiveDotClass() {
        ...
        this.dotsNodes[this.currentSlide].classList.add(GalleryDotActiveClassName);
    }
